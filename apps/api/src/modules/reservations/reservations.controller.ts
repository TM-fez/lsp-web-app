...
export class ReservationsController {
  ...
  confirmReservation = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const dto = req.body as ConfirmReservationDTO;
      const meta = this.getRequestMeta(req);
      res.json(await this.service.confirmWithoutPayment(req.params.id as string, dto, meta, req.activePropertyId));
    } catch (err) {
      next(err);
    }
  };
